...
<div id="floatingBox"
  ...
  <table class="table table-bordered table-sm mb-2" style="width: auto; margin: 0 auto;">
    ...
    <tbody>
      <tr>
        <td class="fw-bold fs-5 bg-white" id="availableLimit"></td>
        ...
        <td class="fw-bold fs-5 bg-white" id="totalLimit"></td>
      </tr>
      <tr>
        <td colspan="3" class="text-center">
          <button onclick="updateLimitAPI(
    document.getElementById('totalLimit').innerText,
    document.getElementById('admin_id').value
  )" type="submit" class="btn btn-sm btn-primary rounded-pill px-3">Submit</button>
        </td>
      </tr>
    </tbody>
  </table>
    ...
    @foreach($Alluser as $record)
        <tr>
            ...
            <td onclick="changeStatus(document.getElementById('status-{{ $record->id }}'),{{ $record->id }})">
                ...
                @else
                    <span id="status-{{ $record->id }}" style="display: inline-block; width: 100px; background-color: #dc3545; color: white; padding: 4px 10px; border-radius: 20px; font-size: 14px; text-align: center; cursor:pointer;">
                        Deactive
                    </span>
                @endif
            </td>
            <td>{{ $record->limit ?? 'N/A' }}</td>
            <td>
                <button onclick="updatelimit('{{ $record->limit }}', '{{ $record->Name }}', '{{ $record->id }}')" class="btn btn-sm btn-success rounded-pill px-3">
                    Add Limit 
                </button>
            </td>
        </tr>
...
//  page link
    ...
    xhr.open("POST", "/muqeemUserStatusChange", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.setRequestHeader('X-CSRF-TOKEN', document.querySelector('meta[name="csrf-token"]').getAttribute('content')); // এই লাইন লাগবে
